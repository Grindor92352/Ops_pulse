...
import { OpsPulse, IssueSeverity } from './index';
...
async function runTests() {
  ...
  const result = await OpsPulse.captureException(
  ...
  const result2 = await OpsPulse.captureException(
    new Error('Payment processing failed'),
  ...
  const result3 = await OpsPulse.captureMessage('User signed up', { severity: IssueSeverity.LOW });
  console.log('Test 3 (message → should succeed):', result3);

  // Test 4: double init guard — should warn, not crash
  OpsPulse.init({ apiKey: 'ANOTHER_KEY' });
  console.log('Test 4: double init → check for [OpsPulse] Already initialized warning above');
}
